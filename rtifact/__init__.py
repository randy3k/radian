...
def main():
    parser = optparse.OptionParser("usage: rtifact")
    ...
    if options.version:
        if r_home:
            ...
            r_binary = "NA"
            r_version = "NA"
        print("rtifact version: {}".format(__version__))
    ...
    from .rtifactapp import RtifactApplication
    RtifactApplication(r_home).run(options)


def deprecated_main():
    print("The program `rice` has been deprecated, use `rtifact` instead.")
    main()
